
CLIENT_ID = 'fed97f3cc7e54e80a39998f04ba17545'

CLIENT_SECRET = 'a551182020f64fed96a94d68a319ccd7'

SPOTIFY_URL = 'https://api.spotify.com/v1/audio-features/'

KEY_HAPPY_WORDS = {'Good', 'Great', 'Happy', 'Happiest', 'Joy', 'Happier'}

KEY_SAD_WORDS = {'Sucks', 'Down', 'Dark', 'Sad', 'Loss', 'low'}

KEY_ANGRY_WORDS = {'Heart Beats', "Rockin' Vibes", 'Rage Beats', 'Dark & Stormy', 'Angry', }

HAPPY_SONGS_FILE_SET = 'data_happy_songs.txt'

ANGRY_SONGS_FILE_SET = 'data_angry_songs.txt'

SAD_SONGS_FILE_SET = 'data_sad_songs.txt'

USER_ID = 'cry2rekkemb4d0ap145a5lyoj'